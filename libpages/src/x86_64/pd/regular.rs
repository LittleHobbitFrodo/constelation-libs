use crate::{PtEntry, PageEntry, PageTable};
use core::ptr::NonNull;



/// A regular pd table entry pointing to a pt table
#[derive(Copy, Clone)]
#[repr(transparent)]
pub struct PdRegularEntry(pub(crate) u64);

impl PageEntry for PdRegularEntry {}
...
impl PdRegularEntry {
    ...
    #[inline]
    pub fn address(&self) -> Option<NonNull<PageTable<PtEntry>>> {
        unsafe {
            core::mem::transmute((self.0 & Self::ADDRESS_MASK) as usize)
        }
    }

    /// Returns the address of the page this entry is pointing to
    #[inline]
    pub fn address_raw(&self) -> *const PageTable<PtEntry> {
        (self.0 & Self::ADDRESS_MASK) as usize as *const _
    }

    /// Returns a reference to the pt table pointed by this entry
    #[inline]
    pub fn pt_table<'pd, 'pt>(&'pd self) -> Option<&'pt PageTable<PtEntry>>
    where 'pd: 'pt {
        unsafe {
