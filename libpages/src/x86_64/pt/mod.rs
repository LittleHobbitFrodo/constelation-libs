use crate::{PageEntry, PageEntryUnion};
...
/// Pt table entry (thread unsafe)
#[derive(Copy, Clone)]
#[repr(transparent)]
pub struct PtEntry(pub(crate) u64);


impl PageEntry for PtEntry {}
impl PageEntryUnion for PtEntry {}

impl PtEntry {

    /// Returns the entry bits as `u64`
    #[inline]
    ...
    /// Constructs new kernel `PtEntry` with given bits
    /// - Returns `None` if the entry is executable and writeable
    pub fn new_kernel_entry(bits: PtKernelBits) -> Option<Self> {
    ...
    /// Construct new kernel `PtEntry` with given bits without validating it
    ///
    /// # Safety
    /// Libpages expects all entrires to be executable **XOR** writeable,
    /// violating this rule will introduce undefined behaviour
    pub const unsafe fn new_kernel_entry_unchecked(bits: PtKernelBits) -> Self {
        Self(bits.as_u64())
    }

    /// Constructs new user `PtEntry` with given bits
    /// - Returns `None` if the entry is executable and writeable
    pub fn new_user_entry(bits: PtUserBits) -> Option<Self> {
        let entry = Self(bits.as_u64());
        if entry.is_executable() && entry.is_writeable() {
            None
        } else {
    ...
    /// Construct new user `PtEntry` with given bits without validating it
    ///
    /// # Safety
    /// Libpages expects all entrires to be executable **XOR** writeable,
    /// violating this rule will introduce undefined behaviour
    pub const unsafe fn new_user_entry_unchecked(bits: PtUserBits) -> Self {
        Self(bits.as_u64())
    }
...
impl core::fmt::Debug for PtEntry {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "PtEntry({}, {}, {}, {}, {}, {}, {}, {}, address: {:p}, key: {:x}, {})",
