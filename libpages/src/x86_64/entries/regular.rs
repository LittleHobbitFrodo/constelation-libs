//! All regular/non-page-sized entries are the same, so they get generated here by a macro

use crate::{KernelBits, PageEntry, PageEntryUnion, PageTable, PdEntry, PdptEntry, PtEntry, UserBits};
use core::ptr::NonNull;



macro_rules! generate_regular_entry {
    ($entry_name:ident, $next_level:ident) => {
        /// A regular entry pointing to a page table
        #[derive(Copy, Clone)]
        #[repr(transparent)]
        pub struct $entry_name(pub(crate) u64);
        ...
        impl $entry_name {
            ...
            pub const ADDRESS_MASK: u64 = 0xFFFFFFFFF000;
            ...
            /// Constructs new kernel `PdRegularEntry` with given bits
            /// - Returns `None` if the entry is executable and writeable
            pub fn new_kernel_entry(bits: KernelBits) -> Option<Self> {
            ...
            /// Construct new kernel `PdRegularEntry` without validating the bits
            ///
            /// # Safety
            /// Libpages expects all entries to be executable **XOR** writeable,
            /// violating this rule may result in undefined behaviour
            pub const unsafe fn new_kernel_entry_unchecked(bits: KernelBits) -> Self {
                Self(bits.as_u64())
            }
            ...
            /// - Returns `None` if the entry is executable and writeable
            pub fn new_user_entry(bits: UserBits) -> Option<Self> {
                let entry = Self(bits.as_u64());
            ...
            /// Construct new user `PdRegularEntry` without validating the bits
            ///
            /// # Safety
            /// Libpages expects all entries to be executable **XOR** writeable,
            /// violating this rule may result in undefined behaviour
            pub const unsafe fn new_user_entry_unchecked(bits: UserBits) -> Self {
                Self(bits.as_u64())
            }
            ...
            #[inline]
            pub fn address(&self) -> Option<NonNull<PageTable<$next_level>>> {
                NonNull::new((self.0 & Self::ADDRESS_MASK) as usize as *mut _)
            }

            /// Returns the address of the page this entry is pointing to
            #[inline]
            pub fn address_raw(&self) -> *const PageTable<$next_level> {
                (self.0 & Self::ADDRESS_MASK) as usize as *const _
            }

            /// Returns a reference to the pt table pointed by this entry
            #[inline]
            pub fn page_table<'pd, 'pt>(&'pd self) -> Option<&'pt PageTable<$next_level>>
            where 'pd: 'pt {
                unsafe {
        ...
        impl core::fmt::Debug for $entry_name {
            fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
                write!(f, "PageEntry({}, {}, {}, {}, {}, {},  address: {:p}, {})",
...
generate_regular_entry!(PdRegularEntry, PtEntry);
generate_regular_entry!(PdptRegularEntry, PdEntry);
generate_regular_entry!(Pml4Entry, PdptEntry);
impl PageEntryUnion for Pml4Entry {}
