use crate::{Either, PageEntry, PageEntryUnion, PdRegularEntry};
...
#[derive(Copy, Clone)]
#[repr(transparent)]
pub struct PdSizedEntry(pub(crate) u64);
...
impl PdSizedEntry {
    ...
    pub const ADDRESS_MASK: u64 = 0xFFFFFFE00000;

    /// The size of the page this entry is pointing to
    /// - 2MB
    pub const PAGE_SIZE: usize = 4096*512;
    ...
    /// Constructs new kernel `PdSizedEntry` with given bits
    /// - Returns `None` if the entry is executable and writeable
    pub fn new_kernel_entry(bits: PdSizedKernelBits) -> Option<Self> {
    ...
    pub const unsafe fn new_kernel_entry_unchecked(bits: PdSizedKernelBits) -> Self {
        Self(bits.as_u64())
    }
    ...
    /// Constructs new kernel `PdSizedEntry` with given bits
    /// - Returns `None` if the entry is executable and writeable
    pub fn new_user_entry(bits: PdSizedUserBits) -> Option<Self> {
        let entry = Self(bits.as_u64());
    ...
    /// Libpages expects all entries to be executable **XOR** writeable,
    /// violating this rule may result in undefined behaviour
    pub const unsafe fn new_user_entry_unchecked(bits: PdSizedUserBits) -> Self {
        Self(bits.as_u64())
    }
...
impl core::fmt::Debug for PdSizedEntry {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "PdSizedEntry({}, {}, {}, {}, {}, {}, {}, {}, address: {:p}, key: {:x}, {})",
